...
const Row = ({ label, value }: { label: string; value: string }) => (
  <div className="flex items-start justify-between gap-3 py-1">
    ...
    <span className="text-[11px] font-medium text-right whitespace-nowrap">{value}</span>
  </div>
);

/** Transaction receipt bubble sent to the customer after a successful transfer. */
export function TransferReceiptCard({ receipt, className = "w-[280px]" }: { receipt: TransferReceipt; className?: string }) {
  return (
    <div className={`${className} rounded-xl border bg-card p-3 shadow-sm`}>
